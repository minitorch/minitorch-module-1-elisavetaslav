...
def test_sigmoid(a: float) -> None:
    ...
    * It is strictly increasing.
    ...
    if abs(a) < 25:
        assert lt(sigmoid(a), sigmoid(a + 1e-4))
...
def test_other(x: float, y: float, z: float) -> None:
    """Test various mathematical properties of the implemented functions.
    Verifies:
    - Double negation returns original value
    - Multiplicative identity property
    - Additive identity property
    - Associative property of addition
    - ReLU non-negativity and correctness
    """
    assert is_close(neg(neg(x)), x)
    assert is_close(mul(x, 1.0), x)
    assert is_close(add(x, 0.0), x)
    assert is_close(add(add(x, y), z),  add(x, add(y, z)))
    assert relu(x) >= 0.0
    assert is_close(relu(x), x if x > 0 else 0.0)
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    """
    sum_separate = add(sum(ls1), sum(ls2))
    combined_sum = sum(addLists(ls1, ls2))
    assert_close(sum_separate, combined_sum)
